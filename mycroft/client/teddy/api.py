...
class TeddyAPI:
    ...
    can be sent to a Mycroft teddy implementation.
    
    Different teddy implementations may implement this differently
    ...
    def system_blink(self, times):
        ...
        """
        self.ws.emit(Message("teddy.system.blink", {'times': times}))

    def eyes_on(self):
        """Illuminate or show the eyes."""
        self.ws.emit(Message("teddy.eyes.on"))

    def eyes_off(self):
        """Turn off or hide the eyes."""
        self.ws.emit(Message("teddy.eyes.off"))
    ...
    def eyes_blink(self, side):
        ...
        """
        self.ws.emit(Message("teddy.eyes.blink", {'side': side}))

    def eyes_narrow(self):
        """Make the eyes look narrow, like a squint"""
        self.ws.emit(Message("teddy.eyes.narrow"))
    ...
    def eyes_look(self, side):
        ...
        Args:
            side (str): 'r' for right
                        ...
                        'c' for crossed
        """
        self.ws.emit(Message("teddy.eyes.look", {'side': side}))
    ...
    def eyes_color(self, r=255, g=255, b=255):
        ...
        """
        self.ws.emit(Message("teddy.eyes.color",
                             {'r': r, 'g': g, 'b': b}))
    ...
    def eyes_brightness(self, level=30):
        ...
        """
        self.ws.emit(Message("teddy.eyes.level", {'level': level}))

    def eyes_reset(self):
        """Restore the eyes to their default (ready) state."""
        self.ws.emit(Message("teddy.eyes.reset"))
    ...
    def eyes_timed_spin(self, length):
        ...
        """
        self.ws.emit(Message("teddy.eyes.timedspin",
                             {'length': length}))
    ...
    def eyes_volume(self, volume):
        ...
        Args:
            volume (int): 0 to 11
        """
        self.ws.emit(Message("teddy.eyes.volume", {'volume': volume}))

    def mouth_reset(self):
        """Restore the mouth display to normal (blank)"""
        self.ws.emit(Message("teddy.mouth.reset"))

    def mouth_talk(self):
        """Show a generic 'talking' animation for non-synched speech"""
        self.ws.emit(Message("teddy.mouth.talk"))

    def mouth_think(self):
        """Show a 'thinking' image or animation"""
        self.ws.emit(Message("teddy.mouth.think"))

    def mouth_listen(self):
        """Show a 'thinking' image or animation"""
        self.ws.emit(Message("teddy.mouth.listen"))

    def mouth_smile(self):
        """Show a 'smile' image or animation"""
        self.ws.emit(Message("teddy.mouth.smile"))
    ...
    def mouth_viseme(self, code):
        ...
        """
        self.ws.emit(Message("teddy.mouth.viseme", {'code': code}))
